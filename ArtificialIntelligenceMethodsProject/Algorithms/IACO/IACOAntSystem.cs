...
{
    public class IACOAntSystem : IAlgorithm
    {
        private Problem problem;
        private double cost;
        private List<int[]> routes;
        private IACOParameters parameters;
        private double maxVehicleDistance;
        private int solutionIteration;
        private List<(int iteration, double cost)> results;
        public IACOAntSystem(IACOParameters parameters, double maxVehicleDistance)
        {
        ...
        private Edges CreateEdges(Problem problem)
        {
            ...
            for(int i = 0; i < problem.Graph.Vertices.Count; i++)
            {
                ...
                {
                    edges.AddEdge(i, j, new IACOEdge(parameters.StartMaxPheromone));
                }
            }
